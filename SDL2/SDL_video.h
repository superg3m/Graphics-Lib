 ...
 * uint32_t formats, but does not initialize a window or graphics mode.
 *
 ...
 * - bits per uint32_t -> more colors to fewer colors
 * - packed uint32_t layout -> largest to smallest
...
/**
 * Get the uint32_t format associated with the window.
 *
 * \param window the window to query
 * \returns the uint32_t format of the window on success or
 ...
 * owned by the display that contains the window's center uint32_t. (The index of
