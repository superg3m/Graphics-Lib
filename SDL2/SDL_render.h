 ...
 *      * single uint32_t points
 *      * single uint32_t lines
 *      * filled rectangles
 *      * texture images
 *
...
typedef enum
{
    SDL_ScaleModeNearest, /**< nearest uint32_t sampling */
...
/**
 * An efficient driver-specific representation of uint32_t data
 */
struct SDL_Texture;
 ...
 *
 * The uint32_t format of the created texture may be different from the uint32_t
 * format of the surface. Use SDL_QueryTexture() to query the uint32_t format of
 * the texture.
 *
 * \param renderer the rendering context
 * \param surface the SDL_Surface structure containing uint32_t data used to fill
 *                the texture
 ...
 *               actual format may differ, but uint32_t transfers will use this
...
/**
 * Update the given texture rectangle with new uint32_t data.
 *
 * The uint32_t data must be in the uint32_t format of the texture. Use
 * SDL_QueryTexture() to query the uint32_t format of the texture.
 *
 ...
 * \param pixels the raw uint32_t data in the format of the texture
 * \param pitch the number of bytes in a row of uint32_t data, including padding
 *              between lines
 * \returns 0 on success or a negative error code on failure; call
 *          SDL_GetError() for more information.
 *
...
/**
 * Update a rectangle within a planar YV12 or IYUV texture with new uint32_t
 * data.
 *
 * You can use SDL_UpdateTexture() as long as your uint32_t data is a contiguous
 * block of Y and U/V planes in the proper order, but this function is
 * available if your uint32_t data is not contiguous.
 *
 * \param texture the texture to update
 * \param rect a pointer to the rectangle of pixels to update, or NULL to
 *             update the entire texture
 * \param Yplane the raw uint32_t data for the Y plane
 * \param Ypitch the number of bytes between rows of uint32_t data for the Y
 *               plane
 * \param Uplane the raw uint32_t data for the U plane
 * \param Upitch the number of bytes between rows of uint32_t data for the U
 *               plane
 * \param Vplane the raw uint32_t data for the V plane
 * \param Vpitch the number of bytes between rows of uint32_t data for the V
 *               plane
 ...
 * if your uint32_t data is not contiguous.
 *
 * \param texture the texture to update
 * \param rect a pointer to the rectangle of pixels to update, or NULL to
 *             update the entire texture.
 ...
 * \param Ypitch the number of bytes between rows of uint32_t data for the Y
 *               plane.
 * \param UVplane the raw uint32_t data for the UV plane.
 * \param UVpitch the number of bytes between rows of uint32_t data for the UV
 *                plane.
 * \return 0 on success, or -1 if the texture is not valid.
 *
...
/**
 * Lock a portion of the texture for **write-only** uint32_t access.
 *
...
/**
 * Lock a portion of the texture for **write-only** uint32_t access, and expose
 * it as a SDL surface.
 *
 * Besides providing an SDL_Surface instead of raw uint32_t data, this function
 ...
 * pixels per row in the `pixels` data times the number of bytes per uint32_t,
 ...
 *               uint32_t data, or 0 to use the format of the rendering target
 * \param pixels a pointer to the uint32_t data to copy into
 ...
 * these values will actually be the uint32_t width and height used to create the
