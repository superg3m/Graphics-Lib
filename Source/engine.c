...
int save_to_ppm_file(Pixel* pixels, size_t width, size_t height, const char* file_path)
{
    int result = 0;
    FILE* f = NULL;
    ...
    char str[3][6]; // A 2D array to store the string representations of each channel (5 digits + 1 null terminator)
